...
with st.sidebar:
    ...
    st.markdown("---")
    st.markdown(
        "**Disclaimer:** This chatbot generates answers using AI based on retrieved module data. "
        "Responses may be incomplete, outdated, or incorrect. Always verify critical academic, "
        "assessment, or course information using official University sources."
    )
    st.markdown("---")
    st.markdown(
        "**Data & Privacy**\n\n"
        "- Your queries and responses may be logged by the hosting server for monitoring and improvement.\n"
        "- Retrieved content is sourced from the local FAISS index; no external web lookups are performed by this app.\n"
        "- Avoid sharing sensitive personal information (e.g., passwords, medical or financial data).\n"
        "- Data retention and deletion follow your organisation's policy."
    )
    ...
    if user_q:
        ...
        with st.chat_message("assistant"):
            try:
                ...
                time.sleep(0.05); progress.progress(20)
                ...
                if not docs:
                    ...
                    st.write("🧱 Building context for the LLM...")
                    context_text = "\n\n".join(d.page_content for d in docs)
                    time.sleep(0.05); progress.progress(80)

                    st.write("🤖 Generating response...")
                    response = chain.invoke({"question": user_q, "context": context_text})
                    progress.progress(100)
                time.sleep(0.05); progress.empty()
                st.write("✅ Done.")
            except Exception as e:
            ...
            with c3:
                ...
                if st.session_state.get(uid):
                    ...
                    st.markdown(f"<script>navigator.clipboard.writeText({response!r});</script>", unsafe_allow_html=True)
                    st.success("Copied answer.")
            with c4:
                uid2 = f"copy_all_{uuid4().hex}"
                ...
                if st.session_state.get(uid2):
                    ...
                    st.markdown(f"<script>navigator.clipboard.writeText({export_md!r});</script>", unsafe_allow_html=True)
                    st.success("Copied answer + sources.")
...
with tab_about:
    ...
    st.markdown("Adds a sidebar disclaimer and data retention information.")
